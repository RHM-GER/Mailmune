...
(id,name,host,port,username,secret_ref,inbox_folder,sent_folder,spam_folder,safety_mode,ollama_model,ollama_validated,ai_enabled,embedding_model,enabled,dry_run,profile_json,last_scan_at,created_at,updated_at,deep_scan_enabled,deep_scan_weekday,deep_scan_hour,last_deep_scan_at)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
...
safety_mode=excluded.safety_mode,ollama_model=excluded.ollama_model,ollama_validated=excluded.ollama_validated,ai_enabled=excluded.ai_enabled,embedding_model=excluded.embedding_model,
...
deep_scan_enabled=excluded.deep_scan_enabled,deep_scan_weekday=excluded.deep_scan_weekday,deep_scan_hour=excluded.deep_scan_hour,last_deep_scan_at=excluded.last_deep_scan_at`,
		...
		account.OllamaValidated, account.AIEnabled, account.EmbeddingModel, account.Enabled, account.DryRun, string(profile), nullableTime(account.LastScanAt),
...
safety_mode,ollama_model,ollama_validated,ai_enabled,embedding_model,enabled,dry_run,profile_json,last_scan_at,created_at,updated_at,
deep_scan_enabled,deep_scan_weekday,deep_scan_hour,last_deep_scan_at FROM accounts ORDER BY name`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.Name, &a.Host, &a.Port, &a.Username, &a.SecretRef, &a.InboxFolder, &a.SentFolder, &a.SpamFolder,
			&a.SafetyMode, &a.OllamaModel, &a.OllamaValidated, &a.AIEnabled, &a.EmbeddingModel, &a.Enabled, &a.DryRun, &profile, &last, &created, &updated,
		...
		if lastDeep.Valid {
			parsed, _ := time.Parse(time.RFC3339Nano, lastDeep.String)
			a.LastDeepScanAt = &parsed
		}
