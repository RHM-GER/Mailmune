...
func testAccount(server *testIMAP) domain.AccountConfig {
	...
	return domain.AccountConfig{ID: "acc", Host: host, Port: port, Username: "user", InboxFolder: "INBOX", SentFolder: "Sent", SpamFolder: "AI_SPAM_FILTER"}
}
...
func collectSync(t *testing.T, client *Client, account domain.AccountConfig, prev *domain.FolderSyncState, opts SyncOptions) (*SyncOutcome, []domain.MessageFeatures) {
	...
	outcome, err := client.SyncFolder(context.Background(), account, "pass", "INBOX", prev, opts, func(features domain.MessageFeatures, _ string) error {
...
func TestInitialSyncIsBoundedAndUIDBased(t *testing.T) {
	server := startTestIMAP(t, moveCaps())
	now := time.Now()
	for i := 0; i < 5; i++ {
	...
	client := NewTestClient(server.clientTemplate, 5*time.Second)
...
func TestSyncFetchesBoundedText(t *testing.T) {
	...
	var text string
	outcome, err := client.SyncFolder(context.Background(), testAccount(server), "pass", "INBOX", nil, SyncOptions{FetchText: true}, func(_ domain.MessageFeatures, body string) error {
		text = body
		return nil
	})
	if err != nil {
...
func TestSyncCancellationStopsRun(t *testing.T) {
	...
	count := 0
	_, err := client.SyncFolder(ctx, testAccount(server), "pass", "INBOX", nil, SyncOptions{}, func(domain.MessageFeatures, string) error {
		count++
		cancel()
		return nil
	})
...
func TestSyncConnectionFailure(t *testing.T) {
	server := startTestIMAP(t, moveCaps())
	client := NewTestClient(server.clientTemplate, 2*time.Second)
	account := testAccount(server)
	_ = server.server.Close()
	time.Sleep(100 * time.Millisecond)
	_, err := client.SyncFolder(context.Background(), account, "pass", "INBOX", nil, SyncOptions{}, func(domain.MessageFeatures, string) error { return nil })
	if err == nil {
...
func TestMoveAtomicMovesMessageAndReportsDestUID(t *testing.T) {
	...
	destination, err := client.MoveAtomic(context.Background(), account, "pass", "INBOX", uid, "AI_SPAM_FILTER")
	if err != nil {
...
func TestMoveRefusedWithoutMoveCapability(t *testing.T) {
	server := startTestIMAP(t, noMoveCaps())
	uid := server.addMessage("INBOX", "spam@example.com", "Spam", "spam", time.Now())
	client := NewTestClient(server.clientTemplate, 5*time.Second)

	_, err := client.MoveAtomic(context.Background(), testAccount(server), "pass", "INBOX", uid, "AI_SPAM_FILTER")
...
func TestTestConnectionReportsCapabilities(t *testing.T) {
	server := startTestIMAP(t, moveCaps())
	client := NewTestClient(server.clientTemplate, 5*time.Second)
	info, err := client.TestConnection(context.Background(), testAccount(server), "pass")
	if err != nil {
		t.Fatal(err)
	}
